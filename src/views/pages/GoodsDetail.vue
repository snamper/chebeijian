  ...
  <div class="GoodsDetail">
    ...
    <div class="title">
      ...
      <div class="col-xs-10">
        ...
        <p class="price" v-if="price_token === null">价格会员可见</p>
        <p class="price" v-else>¥{{goodsInfo.user_price}}</p>
      </div>
    ...
    <div v-if="this.$route.query.company_name">
      ...
      <div class="specification" v-if="checked">
        ...
        <div class="col-xs-10">
          ...
          <p>
            <img :src="httpUrl+'address.png'" alt>
            <span>{{storeInfo.province_name}}{{storeInfo.city_name}}{{storeInfo.district_name}}{{storeInfo.address}}</span>
          </p>
        </div>
      </div>
      <div class="specification" v-if="checked">
        <p class="OrderConfirm">
          安装费用：
          <span class="price">¥{{installPrice}}</span>
        </p>
      </div>
    </div>
    <div class="specification two" v-if="checked">
      <p class="OrderConfirm" @click="goChooseStore">
        ...
        <span>
          <img data-v-086081d3 :src="httpUrl+'jtright.png'" alt>
        </span>
      </p>
    </div>
    <div class="shadow"></div>
        ...
        <div class="main">
          ...
          <div class="divTab" v-show="nowIndex===0">
            <div class="detail">
              ...
              <p>模块编码：{{goodsInfo.module_coding}}</p>
          ...
          <div class="divTab" v-show="nowIndex===2">
            ...
            <div
              @click="getMore()"
              style="font-size:16px;text-align: center;padding-bottom: 20px;"
              v-if="!noMore"
            >点击加载更多...</div>
            <div style="font-size:16px;text-align: center;padding-bottom: 20px;" v-if="noMore">没有更多了</div>
          </div>
        </div>
        <!-- end 商品内容 -->
      </div>
    ...
    <div class="footer">
      <div class="col-xs-4">
        <div class="col-xs-6" @click="show3=true">
          <p>
            <img data-v-086081d3 :src="httpUrl+'zixun.png'" alt>
          </p>
          <p>电话咨询</p>
        </div>
        <div class="col-xs-6" @click="show4=true">
          <p>
            <img data-v-086081d3 :src="httpUrl+'kefu.png'" alt>
          </p>
          <p>在线客服</p>
        </div>
      </div>
    ...
    <div class="popup" v-show="show">
      ...
      <div class="popuo_footer">
        <button class="cancel" @click="show=false">继续采购</button>
        <button class="ensure" @click="goCartList">去购物车结算</button>
      </div>
    </div>
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    if (fromURL == "登录" && to.name == "登录") {
      next(window.history.go(-1)); //当按返回键是C--B--C 情形时，直接跳转至A
    } else {
      next(); //默认跳转
    }
  },
  methods: {
          ...
          axios
            ...
            .then(function(response) {
              ...
              if (response.data.errorCode === 0) {
                ...
                window.localStorage.setItem("ensureOrderData", JSON.stringify(product));
                _this.$router.push({
                  path: "OrderConfirm"
    ...
    initData() {
      ...
      console.log(_this.$route.query.collect);
      if (_this.$route.query.collect == "o") {
        this.addCollect();
      }
    },
    // 添加到购物车方法
    addCar() {
      ...
      // 判断安装按钮是否为选中状态
      // 是
      if(_this.price_token==null){
        Toast("请先登录");
        setTimeout(() => {
          _this.$router.push({
            path: "/login",
            query: {
              redirect: _this.$router.currentRoute.fullPath + "&collect=o"
            }
          });
        }, 1000);
      }else{
    ...
    getMore() {
      ...
      axios
        ...
        .then(function(response) {
          if (response.data.errorCode == 0) {
            _this.cartModel = _this.cartModel.concat(
