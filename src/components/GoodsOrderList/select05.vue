  ...
  <div>
    <van-list v-model="loading" :finished="finished" @load="onLoad">
      ...
      <van-row class="title row">
        ...
        <!-- 订单状态 -->
        <van-col span="6" class="state" v-if="order.is_install == 0 && order.status == 2">待收货</van-col>
      </van-row>
      ...
      <div class="goods">
        <ul>
          <li>
            <router-link :to="{path:'GoodsOrderDetail',query:{id:order.id,active:4}}">
            <div class="col-xs-4">
    ...
    <div class="popup" v-show="show5">
      ...
      <div class="popuo_footer">
        <button class="cancel" @click="show5=false">取消</button><button class="ensure" @click="modify(id)">确定</button>
      </div>
    </div>
  ...
  methods: {
    onLoad() {
      ...
      setTimeout(() => {
        axios
          .post(
            _this.apihttpUrl + 'api/user/product/order/list?platform=weixin&version=1000&deliver_goods_status=22&row=15&page=' +
              page,
            qs.stringify({
