  ...
  <div>
    <van-list v-model="loading" :finished="finished" @load="onLoad">
      ...
      <div class="goods">
        <ul>
          <li>
            <router-link :to="{path:'GoodsOrderDetail',query:{id:order.id,active:2}}">
            <div class="col-xs-4">
  ...
  methods: {
    onLoad() {
      ...
      setTimeout(() => {
        axios
          .post(
            _this.apihttpUrl + 'api/user/product/order/list?platform=weixin&version=1000&status=wait_confirm&row=15&page=' +
              page,
            qs.stringify({
